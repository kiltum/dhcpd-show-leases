...
def parse_leases_file(leases_file):
        ...
        for line in leases_file:
                ...
                if key == 'lease':
                        ...
                        if value.startswith('ddns-fwd-name'):
                                lease_rec['ddns-fwd-name'] = value.split()[2].replace('"', '')
...
for lease in report_dataset:
        if 'ddns-fwd-name' in lease:
                if len(lease['client-hostname']) > 0:
                       name = lease['client-hostname'] + ' (' + lease['ddns-fwd-name'] + ')'
                else:
                       name = lease['ddns-fwd-name']
        else:
